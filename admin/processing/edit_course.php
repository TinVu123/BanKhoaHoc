...
if(isset($_POST['submit_change_course'])){
    ...
    if($_FILES['image']['size'] != 0){
        ...
        $add_course_query = mysqli_query($conn, 
                            "UPDATE `courses` SET
                            image = '$image', 
                            name =  '$name_course', 
                            price = '$price_course', 
                            ...
                            WHERE id = $id_course ;") or die('query failed');
        if($add_course_query){
...
if(isset($_POST['submit_video'])){
    ...
    if($_FILES['input_video']['size'] <= 0){
        ...
        $add_course_query = mysqli_query($conn,"INSERT INTO `course_videos`(course_id,video,title,duration) VALUES('$id_course','$video','$video_name','$duration');") or die('query failed');
...
if(isset($_POST['submit_save_change_lesson'])){
    ...
    if($_FILES['input_video']['size'] > 0){
        ...
        $add_video_query = mysqli_query($conn,"UPDATE  `course_videos` SET title = '$newName', video ='$video' WHERE id = '$id_old_video';") or die('query failed');
        if($add_video_query){
            move_uploaded_file($video_tmp_name,$video_folder );
            $message = "Thêm video thành công!";
        }
        ...
        mysqli_query($conn,"UPDATE  `course_videos` SET title = '$newName' WHERE id = '$id_old_video';") or die('query failed');
    }
    // header('Location:../admin_course_edit.php?id='.$id_course);
